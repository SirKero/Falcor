...
def render_graph_RayTracedSoftShadows():
    ...
    g.create_pass('TM_NRD_Sigma', 'ToneMapper', {'outputSize': 'Default', 'useSceneMetadata': True, 'exposureCompensation': 0.0, 'autoExposure': False, 'filmSpeed': 100.0, 'whiteBalance': False, 'whitePoint': 6500.0, 'operator': 'Aces', 'clamp': True, 'whiteMaxLuminance': 1.0, 'whiteScale': 11.199999809265137, 'fNumber': 1.0, 'shutter': 1.0, 'exposureMode': 'AperturePriority'})
    g.create_pass('ImageEquation', 'ImageEquation', {'formula': 'I0[xy] * I0[xy] * I1[xy]', 'format': 'RGBA32Float'})
    ...
    g.create_pass('TM_Default', 'ToneMapper', {'outputSize': 'Default', 'useSceneMetadata': True, 'exposureCompensation': 0.0, 'autoExposure': False, 'filmSpeed': 100.0, 'whiteBalance': False, 'whitePoint': 6500.0, 'operator': 'Aces', 'clamp': True, 'whiteMaxLuminance': 1.0, 'whiteScale': 11.199999809265137, 'fNumber': 1.0, 'shutter': 1.0, 'exposureMode': 'AperturePriority'})
    ...
    g.add_edge('ImageEquation.out', 'TM_NRD_Sigma.src')
    g.add_edge('ModulateIllumination.output', 'TM_NRD_Full.src')
    g.add_edge('AccumulatePass.output', 'TM_Default.src')
    g.add_edge('RayTracedSoftShadows.color', 'AccumulatePass.input')
    g.mark_output('TM_Default.dst')
    g.mark_output('TM_NRD_Full.dst')
    g.mark_output('TM_NRD_Sigma.dst')
    return g
